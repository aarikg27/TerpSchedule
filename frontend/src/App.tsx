...
function readSharedLink(): { term?: string; schedule?: RankedSchedule; error?: string } {
  ...
  try {
    ...
    if (shared.schedule?.sections?.length && shared.term) return { term: shared.term, schedule: shared.schedule };
  } catch { /* handled below */ }
  return { error: 'This shared schedule link is invalid or incomplete.' };
}

function legalPageFromPath(pathname: string): LegalPage | null {
...
export const App: React.FC = () => {
  const [initialShare] = useState(readSharedLink);
  const [term, setTerm] = useState(initialShare.term || '202608');
  ...
  const [activeSchedule, setActiveSchedule] = useState<RankedSchedule | null>(initialShare.schedule || null);
  const [loading, setLoading] = useState<boolean>(false);
  const [apiReady, setApiReady] = useState(false);
  const [generationStartedAt, setGenerationStartedAt] = useState<number | null>(null);
  const [generationElapsed, setGenerationElapsed] = useState(0);
  const [error, setError] = useState<string | null>(initialShare.error || null);
  ...
  const [legalPage, setLegalPage] = useState<LegalPage | null>(() => legalPageFromPath(window.location.pathname));
  const [page, setPage] = useState<'landing' | 'planner'>(() => window.location.pathname === '/planner' ? 'planner' : 'landing');
  const legalReturnPath = useRef(window.location.pathname === '/planner' ? '/planner' : '/');
  ...
  useEffect(() => {
    const onPopState = () => {
      setPage(window.location.pathname === '/planner' ? 'planner' : 'landing');
      setLegalPage(legalPageFromPath(window.location.pathname));
    };
  ...
  useEffect(() => {
    ...
    void Promise.resolve().then(async () => {
      ...
      if (!workspaceUserId || !neonClient) { setWorkspaceReady(true); return; }
      try {
        const state = await loadPlannerState();
        if (cancelled) return;
        if (Array.isArray(state?.selectedCourses)) setSelectedCourses((state.selectedCourses as string[]).slice(0, 8));
      ...
      } catch { /* start with local defaults when cloud sync is unavailable */ }
  ...
  useEffect(() => {
    if (!workspaceReady || !workspaceUserId || !neonClient) return;
    const timer = window.setTimeout(() => {
      savePlannerState(workspaceUserId, { selectedCourses, term, constraints, preferenceRanking }).catch(() => undefined);
    }, 700);
    return () => window.clearTimeout(timer);
  }, [workspaceReady, workspaceUserId, selectedCourses, term, constraints, preferenceRanking]);

  const openPlanner = () => { window.history.pushState({}, '', '/planner'); setLegalPage(null); setPage('planner'); window.scrollTo(0, 0); };
  const openLanding = () => { window.history.pushState({}, '', '/'); setLegalPage(null); setPage('landing'); window.scrollTo(0, 0); };
  const openLegalPage = (nextPage: LegalPage) => {
    legalReturnPath.current = page === 'planner' ? '/planner' : '/';
    window.history.pushState({}, '', `/${nextPage}`);
    setLegalPage(nextPage);
  };
  const closeLegalPage = () => {
    window.history.replaceState({}, '', legalReturnPath.current);
    setLegalPage(null);
  };
  ...
  const handleAddCourse = (courseId: string) => {
    if (selectedCourses.length >= 8) {
      setError('You can compare up to 8 courses at once. Remove one before adding another.');
      return;
    }
  ...
  }, [selectedCourses, term, constraints, weights, preferenceRanking]);
  ...
  if (page === 'landing') return <><LandingPage onStart={openPlanner} theme={theme} onToggleTheme={cycleLandingTheme} onLegalPage={openLegalPage} />{legalPage && <LegalDialog page={legalPage} onClose={closeLegalPage} />}</>;
  ...
  return (
    <div className="app-shell min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
      ...
      <div className="lg:hidden flex border-b border-slate-800 bg-slate-900 px-4 py-2 gap-2 text-xs">
        <button
          ...
          className={`flex-1 py-1.5 rounded-lg font-bold flex items-center justify-center gap-1.5 ${
            mobileTab === 'inputs' ? 'bg-red-700 text-white' : 'text-slate-400'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-1.5 rounded-lg font-bold flex items-center justify-center gap-1.5 ${
            mobileTab === 'grid' ? 'bg-red-700 text-white' : 'text-slate-400'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-1.5 rounded-lg font-bold flex items-center justify-center gap-1.5 ${
            mobileTab === 'ranking' ? 'bg-red-700 text-white' : 'text-slate-400'
          }`}
        >
      ...
      <main className="flex-1 max-w-[1500px] w-full mx-auto p-4 lg:p-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          >
            ...
            />
            <DegreeAuditImporter onAddCourses={(courses) => setSelectedCourses((current) => [...new Set([...current, ...courses])].slice(0, 8))} />
      ...
      <footer className="mx-auto flex w-full max-w-[1500px] flex-wrap items-center justify-between gap-3 px-5 pb-6 text-[11px] text-slate-500">
        ...
        <span className="flex gap-4"><button type="button" onClick={() => openLegalPage('privacy')} className="hover:text-slate-800">Privacy</button><button type="button" onClick={() => openLegalPage('terms')} className="hover:text-slate-800">Terms & disclaimer</button></span>
      </footer>
      {legalPage && <LegalDialog page={legalPage} onClose={closeLegalPage} />}
