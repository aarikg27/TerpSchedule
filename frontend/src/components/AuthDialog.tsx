...
export const AuthDialog: React.FC<{ mode: 'sign-in' | 'sign-up'; onClose: () => void; onLegalPage: (page: LegalPage) => void }> = ({ mode, onClose, onLegalPage }) => {
  ...
  return createPortal(<div className="auth-backdrop fixed inset-0 z-[1000] flex items-start justify-center overflow-y-auto bg-black/45 p-4 backdrop-blur-sm sm:py-8" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <div ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-label={mode === 'sign-in' ? 'Sign in to TerpSchedule' : 'Create a TerpSchedule account'} className="auth-dialog relative my-auto w-full max-w-md rounded-[28px] border border-black/10 bg-white p-6 shadow-2xl outline-none sm:p-8">
      ...
      <button type="button" disabled={busy} onClick={google} className="auth-google mt-6 flex w-full items-center justify-center gap-2 rounded-xl border px-4 py-3 text-sm font-semibold shadow-sm disabled:opacity-50"><span className="text-base font-bold text-blue-600">G</span> Continue with Google</button>
      ...
      <form onSubmit={submit} className="space-y-3">
        ...
        <button disabled={busy} className="auth-primary w-full rounded-xl px-4 py-3 text-sm font-semibold text-white disabled:opacity-50">{busy ? 'Please wait…' : mode === 'sign-in' ? 'Sign in' : 'Create account'}</button>
      </form>
