    ...
    <main>
      <section className="mx-auto grid max-w-7xl items-center gap-14 px-5 pb-20 pt-14 sm:px-8 lg:grid-cols-[1.05fr_.95fr] lg:pb-28 lg:pt-24">
        <div>
          ...
          <h1 className="max-w-3xl text-5xl font-semibold leading-[.98] tracking-[-.055em] sm:text-7xl">Your semester,<br/><span className="text-slate-500">without the spreadsheet.</span></h1>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3"><button type="button" onClick={onStart} className="group flex items-center gap-2 rounded-full bg-red-600 px-6 py-3.5 text-sm font-semibold text-white shadow-lg shadow-red-600/20 hover:bg-red-500">Start planning <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5"/></button><span className="text-xs text-slate-600">No account required</span></div>
          <div className="mt-8 flex flex-wrap gap-x-5 gap-y-2 text-xs text-slate-600">{['Live Testudo sections','Open-seat filtering','Walking estimates'].map(item => <span key={item} className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-emerald-600"/>{item}</span>)}</div>
        </div>
