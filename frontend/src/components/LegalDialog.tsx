...
const ContactLink = () => CONTACT_EMAIL
  ? <a className="font-medium text-blue-600 underline" href={`mailto:${CONTACT_EMAIL}`}>{CONTACT_EMAIL}</a>
  : <a className="font-medium text-blue-600 underline" href="https://github.com/aarikg27" target="_blank" rel="noreferrer">the project operator</a>;
    ...
    <article ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-label={page === 'privacy' ? 'Privacy policy' : 'Terms and disclaimer'} className="legal-dialog max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-[28px] border border-black/10 bg-white p-6 shadow-2xl outline-none sm:p-8">
      ...
      {page === 'privacy' ? (
        <div className="mt-6 space-y-4 text-sm leading-6 text-slate-600">
          <p className="text-xs text-slate-500"><strong>Effective and last updated:</strong> August 26, 2026</p>
          ...
          <section><h3 className="font-semibold text-slate-900">Technical information</h3><p className="mt-1">Cloudflare, Render, and Neon may process IP addresses, device/browser information, cookies or authentication tokens, request timestamps, and diagnostic logs to deliver, secure, and troubleshoot the service. TerpSchedule logs request paths, response status, and duration and temporarily uses network addresses for rate limiting. Outside information already present in a degree-audit upload, it does not request device location, a student ID, Testudo credentials, or registration credentials.</p></section>
          ...
          <section><h3 className="font-semibold text-slate-900">Your choices and contact</h3><p className="mt-1">You may use the service as a guest, clear an individual audit or schedule, clear all synced planning data, or delete your account from Account &amp; data settings. For an access, correction, deletion, or privacy question, contact <ContactLink/>.</p></section>
        </div>
      ) : (
        <div className="mt-6 space-y-4 text-sm leading-6 text-slate-600">
          <p className="text-xs text-slate-500"><strong>Effective and last updated:</strong> August 26, 2026</p>
          ...
          <section><h3 className="font-semibold text-slate-900">Unofficial planning tool</h3><p className="mt-1">TerpSchedule is an independent student project and is not affiliated with, sponsored by, or endorsed by the University of Maryland. Testudo, the official UMD degree audit, and university advisors are authoritative.</p></section>
          ...
          <section><h3 className="font-semibold text-slate-900">Accounts and availability</h3><p className="mt-1">Accounts are optional. You are responsible for protecting your credentials. The service may change, experience delays, lose availability, or be discontinued. Free hosting may introduce cold-start delays. You may delete your account from Account &amp; data settings.</p></section>
          <section><h3 className="font-semibold text-slate-900">Disclaimer and liability</h3><p className="mt-1">To the maximum extent permitted by law, TerpSchedule is provided “as is” and “as available,” without warranties of accuracy, availability, fitness for a particular purpose, or noninfringement. The project operator is not liable for registration outcomes, missed requirements, schedule conflicts, lost data, or indirect or consequential damages resulting from use of the service. Nothing here excludes rights or liability that cannot legally be excluded.</p></section>
          ...
          <section><h3 className="font-semibold text-slate-900">Contact</h3><p className="mt-1">Questions, reports, and legal notices may be sent through <ContactLink/>.</p></section>
        </div>
      )}
    </article>
  </div>, document.body);
